...
// 아파트의 개수 N, 현재 기지국이 설치된 아파트의 번호가 담긴 1차원 배열 stations, 전파의 도달 거리 W가 매개변수로 주어질 때, 모든 아파트에 전파를 전달하기 위해 
...
class Solution {
    public int solution(int n, int[] stations, int w) {
        int answer = 0;
        int position = 1; // 아파트 시작 위치
        int index = 0;    // 기지국 인덱스

        while (position <= n) {
            // 현재 위치가 기지국 범위 내이면 해당 범위 넘어감
            if (index < stations.length && position >= stations[index] - w) {
                position = stations[index] + w + 1;
                index++;
            } else {
                // 커버되지 않은 구간이므로 5G 기지국 설치
                answer++;
                position += w * 2 + 1; // 새 기지국 커버 범위만큼 전진
            }
        }
